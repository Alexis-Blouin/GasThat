...
namespace FollowCamera // Or any other appropriate namespace
{
    public class CameraFollow : MonoBehaviour
    {
        ...
        {
            // Get mouse input
            var mouseDelta = Mouse.current.delta.ReadValue() * Time.smoothDeltaTime;
            var targetMouseDelta = new Vector2(mouseDelta.x * mouseSensitivityX, mouseDelta.y * mouseSensitivityY);

            // Apply aim sensitivity multiplier when aiming
            if (isAiming)
            {
                targetMouseDelta *= aimSensitivityMultiplier;
            }

            // Invert Y axis if needed
            if (invertMouseY)
                targetMouseDelta.x = -targetMouseDelta.y;
            ...
            yRotation += targetMouseDelta.x;

            // Vertical rotation (X axis) - rotates the camera
            xRotation -= targetMouseDelta.y;
